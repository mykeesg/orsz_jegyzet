...
\begin{document}
	...
	\begin{figure}[!ht]
		...
		% color sets
		\fill[gray, opacity=0.1] \firstset;
	...
	\begin{figure}[!ht]
		...
		% define sets
		\def\firstset{(3,3) circle (3)}
		\def\secondset{(6.5,3) circle (3)}		
		
		% color (first \ second) set
		...
		\fill[gray, opacity=0.2] \firstset;
		\end{scope}
...
\paragraph{}
	...
	\subsubsection{Aszinkron, közös memóriás architektúra}
	\begin{figure}[h!]
		\centering
		\begin{tikzpicture} [
			emptyblock/.style={rectangle, text width=4.5cm, line width=2pt, minimum height=1.3cm, align=center},
			wideblock/.style={emptyblock, text width=12cm, draw},
			memblock/.style={emptyblock, text width=2cm, draw},
			procblock/.style={emptyblock, text width=1.3cm, draw},
			clockblock/.style={emptyblock, minimum height=0.8cm, text width=0.8cm, draw},
			normline/.style={>=latex, line width=2, black}
		]
			\node[wideblock] (memory) {};
			\node[left=0.5cm of memory] (memlabel) {\large $M$};
			\node[memblock, align=center, right=0cm of memory.west] (mem1) {\large $M_1$};
			\node[memblock, align=center, left=3cm of memory.east] (memi) {\large $M_i$};
			\node[emptyblock, align=center] (memdots) at ($(mem1)!0.5!(memi)$) {\large $\dots$};
			\node[emptyblock, align=center] (memdots2) at ($(memi.east)!0.5!(memory.east)$) {\large $\dots$};
			
			\node[procblock, above=2cm of mem1] (proc1) {\large $P_1$};
			\node[procblock, above=2cm of memi] (prock) {\large $P_k$};
			
			\node[clockblock, above=of proc1] (clock1) {\large $c_1$};
			\node[clockblock, above=of prock] (clockk) {\large $c_k$};
			
			\node[emptyblock, above=0cm of $(proc1)!0.5!(prock)$] (procdots) {\large $\dots$};
			
			\draw[normline] (mem1) -- (proc1) node[midway, right] {\large r/w};
			\draw[normline] ($(memi.north) - (0.5, 0)$) -- (proc1) node[midway, above] {\large r};
			\draw[normline] (memi) -- (prock) node[midway, right] {\large r/w};
			\draw[normline] (clock1) -- (proc1);
			\draw[normline] (clockk) -- (prock);
		\end{tikzpicture}
		\caption{Aszinkron, közös memóriás architektúra}
		\label{fig:async}
	\end{figure}
	
	\begin{itemize}
		\item $M$ közös memória, ``rekeszeit'' indexeljük.
		\item $P_i$-k processzorok, mindegyik külön $c_i$ órajellel rendelkezik.
		\item Adott $S=(s_0, \{s_1, \dots, s_n\})$ program minden $s_i$ utasítását egy-egy processzorhoz rendeljük, amely őt fogja futtatni, de egy processzorhoz többet is rendelhetünk: $s_i \rightarrow P_j$.
	\end{itemize}

	Leképezés:
	\begin{itemize}
		\item $s_i$-t a $P_j\ \infty$  sokszor hajtsa végre.
		\item $P_j$-nek legyen írási joga $VL(s_i)$ területeihez.
		\item $P_j$-nek legyen olvasási joga $VR(s_i)$ területeihez.
		\item Ha $s_i$ szimultán értékadás, akkor az legyen atomi.
	\end{itemize}

	
	\subsubsection{Elosztott rendszer}
	\begin{figure}[h!]
		\centering
		\begin{tikzpicture} [
			emptyblock/.style={rectangle, text width=4.5cm, line width=2pt, minimum height=1.3cm, align=center},
			wideblock/.style={emptyblock, text width=12cm},
			memblock/.style={emptyblock, text width=2cm, draw},
			procblock/.style={emptyblock, text width=1.3cm, draw},
			clockblock/.style={emptyblock, minimum height=0.8cm, text width=0.8cm, draw},
			normline/.style={>=latex, line width=2, black}
		];
		\node[memblock, align=center] (mem1) {\large $M_1$};
		\node[memblock, align=center, right=3cm of mem1] (memk) {\large $M_k$};
		
		\node[procblock, above=of mem1] (proc1) {\large $P_1$};
		\node[procblock, above=of memk] (prock) {\large $P_k$};
		
		\node[clockblock, above=of proc1] (clock1) {\large $c_1$};
		\node[clockblock, above=of prock] (clockk) {\large $c_k$};
		
		\node[emptyblock, align=center] (dots) at ($(mem1)!0.5!(memk)$) {\large $\dots$};
				
		\draw[normline] (mem1) -- (proc1);
		\draw[normline] (memk) -- (prock);
		\draw[normline] (clock1) -- (proc1);
		\draw[normline] (clockk) -- (prock);
		
		\draw[normline, ->] (proc1) -- (prock) node[midway, above] {\large $c_{1k}$};
		\end{tikzpicture}
		\caption{Elosztott rendszer}
		\label{fig:distributed}
	\end{figure}
	
	\begin{itemize}
		\item Minden processzorhoz külön órajel és külön memória tartozik, egymásét nem látják.
		\item Kommunikáció csatornákon keresztül történik, melyekhez tartozhat FIFO puffer.
	\end{itemize}
	...
	\subsubsection{Szinkron, közös memóriás architektúra}
	\begin{figure}[h!]
		\centering
		\begin{tikzpicture} [
			emptyblock/.style={rectangle, text width=4.5cm, line width=2pt, minimum height=1.3cm, align=center},
		...
		\node[clockblock, above=of $(proc1.north)!0.5!(prock.north)$] (clock) {\large $c$};
		
		\node[emptyblock] (procdots) at ($(proc1)!0.5!(prock)$) {\large $\dots$};
		
		\draw[normline] (mem1) -- (proc1) node[midway, right] {\large r/w};
		\draw[normline] ($(memi.north) - (0.5, 0)$) -- (proc1) node[midway, above] {\large r};
		\draw[normline] (memi) -- (prock) node[midway, right] {\large r/w};
		\draw[normline] (clock) -- (proc1);
		\draw[normline] (clock) -- (prock);
		\end{tikzpicture}
		\caption{Szinkron, közös memóriás architektúra}
		\label{fig:async}
	\end{figure}
